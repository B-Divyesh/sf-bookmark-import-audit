...
const pageMeta: Record<string, [string, string]> = {
  '/': ['Bookmark Import Audit — check bookmark imports', 'Check a bookmark HTML file for folder merges, duplicate links, URL variants, and missing titles before importing.'],
...
function footer(): string { return `<footer><p><strong>Bookmark Import Audit</strong> checks bookmark HTML files before import.</p><div class="footer-meta"><nav aria-label="Legal"><a href="/privacy" data-route>Privacy</a><a href="/terms" data-route>Terms</a></nav><span>Built by Param Factory</span><span>build 1.0.0-r3</span></div></footer>`; }
...
function collisionMarkup(collision: FolderCollision): string { return `<li class="finding"><div class="finding-head"><span class="status danger">May merge</span><strong>“${esc(collision.title)}” appears in ${collision.paths.length} folders</strong></div><ul class="trace-list">${collision.paths.map((path) => `<li>${pathLabel(path)}</li>`).join('')}</ul></li>`; }
...
function resultMarkup(document: AuditDocument): string {
  ...
  return `<section class="results reveal" aria-labelledby="result-title"><div class="result-header"><div><p class="eyebrow">Audit complete</p><h2 id="result-title">${count} issues found</h2><p>${esc(document.fileName)} · checked locally</p></div><span class="seal review">Review</span></div><div class="gauges" aria-label="Audit counts"><div><span>${document.bookmarks.length}</span><b>Bookmarks</b></div><div><span>${document.folders.length}</span><b>Folders</b></div><div><span>${result.maxDepth}</span><b>Levels deep</b></div><div><span>${count}</span><b>Issues</b></div></div>${result.folderCollisions.length ? `<section class="finding-section"><div class="section-number">01</div><div><h3>Folders that may merge</h3><p>Some bookmark apps combine same-named folders. These full folder paths stay separate in the export.</p><ol class="finding-list">${result.folderCollisions.map(collisionMarkup).join('')}</ol></div></section>` : ''}${result.duplicateClusters.length ? `<section class="finding-section"><div class="section-number">02</div><div><h3>Duplicate links</h3><p>Links are grouped after removing tracking details and the # fragment. Every original link stays in the export.</p><ol class="finding-list">${result.duplicateClusters.map((cluster) => clusterMarkup(cluster, 'duplicate')).join('')}</ol></div></section>` : ''}${result.variantClusters.length ? `<section class="finding-section"><div class="section-number">03</div><div><h3>Likely URL variants</h3><p>These differ by http, https, www, or a known redirect link. Verify them manually.</p><ol class="finding-list">${result.variantClusters.map((cluster) => clusterMarkup(cluster, 'variant')).join('')}</ol></div></section>` : ''}${result.missingTitles.length || result.invalidUrls.length ? `<section class="finding-section"><div class="section-number">04</div><div><h3>Missing titles and malformed URLs</h3><p>Blank titles get a hostname fallback. Malformed URLs remain visible for repair.</p><ul class="finding-list">${result.missingTitles.map((b) => missingMarkup(b)).join('')}${result.invalidUrls.map((b) => missingMarkup(b, true)).join('')}</ul></div></section>` : ''}<section class="repair-section"><div><p class="eyebrow">Corrected copy</p><h3>Export a corrected copy</h3><p>The download is separate. It changes only folder names that may merge and fills blank titles.</p></div>${ledger.length ? `<div class="ledger"><table><thead><tr><th>Original folder</th><th>Exported folder</th></tr></thead><tbody>${ledger.map((item) => `<tr><td data-label="Original">${esc(item.before)}</td><td data-label="Exported">${esc(item.after)}</td></tr>`).join('')}</tbody></table></div>` : ''}<div class="export-actions"><button class="button primary" id="export-html">Export corrected HTML</button><button class="button secondary" id="export-csv">Export review CSV</button><button class="button quiet" id="forget-audit">Forget this audit</button></div></section></section>`;
}
